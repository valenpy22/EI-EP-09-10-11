...
sobrepeso <- hombres %>% filter(EN == "Sobrepeso") %>% sample_n(45)
nosobrepeso <- hombres %>% filter(EN == "No sobrepeso") %>% sample_n(45)

muestra <- rbind(sobrepeso, nosobrepeso)

set_construccion <- rbind(
  sample_n(muestra %>% filter(EN == "Sobrepeso"), 30),
  sample_n(muestra %>% filter(EN == "No sobrepeso"), 30)
)
